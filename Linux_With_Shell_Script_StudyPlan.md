...
`` one of the nice feature in lvm is logical volume resizing ,we can increase the logical volume ,sometime without any downtime and we can migrate data from failing hard disk creating mirrors and snapshot. ``
 ------------------------------------------LVM-------------------------
...
>> Ans) When we reduce the LV before we must unmount that  (If filesystem is not mounted we can't see filesin the mounted dir)
 ...
 ###  Steps : need a downtime to reduce the filesystem ( We can reduce the ext234 file but we cant reduce xfs)
